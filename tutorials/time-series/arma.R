...
p <- as.data.frame(readr::read_csv("/Users/trenthenderson/Downloads/preds.csv")) %>%
  dplyr::select(c(phi, theta)) %>%
  mutate(iteration = row_number()) %>%
  pivot_longer(cols = 1:2, names_to = "parameter", values_to = "values") %>%
  ggplot(aes(x = values)) +
  stat_halfeye(fill = "grey90") +
  geom_vline(data = true_params, xintercept = value, lty = "dashed", size = 0.7)
  labs(title = "Parameter posterior distributions",
       subtitle = "True parameter values of mu = 5, phi = 0.5, and theta = 0.2 are plotted as vertical dashed lines.",
       x = "Value",
       y = "Posterior density") +
  theme_bw() +
